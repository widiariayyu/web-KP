...
class KPSKController extends Controller
{
    ...
    public function dtKP(KPSK $sk)
    {
        $datas = Txkp::where('no_sk_gub', $sk->SKPenyelenggaraan)->with('statusawalkendaraan');
    ...
    public function kpStore(Request $request)
    {
        ...
        try {
            ...
            $master =Txkp::create([
                ...
                'no_kp' => $setting->nomor_surat_kp_1.'.'.$request->NoInduk.'/'.$request->nolambung.'/ASK.'.$request->perusahaan.'/DISHUB-'.$request->tahun,
                'tgl1' => $request->tglsk,
                'tgl2' => $tgl[1],
                'tgl3' => $tgl[2],
                'tgl4' => $tgl[3],
