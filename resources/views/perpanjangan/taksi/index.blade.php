...
@section('content_header')
  <h1>Perpanjangan / Daftar Ulang Angkutan Sewa Taksi</h1>
  <ol class="breadcrumb">
...
<script type="text/javascript">
    ...
    $('#nokartu').focus();
	  var table = $('#table').DataTable({
        ...
        info: false,
        searching: false,
	      ajax: '{!! url('perpanjangan/taksi/detail') !!}',
	      columns: [
    ...
    function dataload(id) {
      var url;
      if (id != '') {
        $.get("{{url('perpanjangan/taksi/getdata')}}/"+id, function(data){
          if (data.nolambung) {
            url = "{{url('perpanjangan/taksi/detail')}}/"+id;
            ...
            url = '{!! url('perpanjangan/taksi/detail') !!}';
            table.ajax.url(url).load();
    ...
    $('#bayar').click(function(e) {
      var id = detilid;
      if (id != '') {
        $.get("{{url('perpanjangan/taksi/bayar')}}/"+id, function(data){
          if (data != 'gagal') {
            console.log(data)
            dataload(data);
