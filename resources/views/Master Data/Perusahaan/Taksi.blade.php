...
<script type="text/javascript">
    ...
    var id;
	  var table = $('#table').DataTable({
	      processing: true,
	      serverSide: true,
	      ajax: '{{ route('taksi.perusahaan.get') }}',
	      columns: [
          { data: 'id', name: 'id' },
    ...
    $('#btntambah').on('click', function(){
      ...
      $('#modal form').attr('action', "{{ route('taksi.perusahaan.store') }}");
    ...
    $('#table tbody').on('click', '#btnedit', function () {
      ...
      $('#modal form').attr('action', "{{ URL('/m/perusahaantaksi') }}/" + id);
