...
<body>
  <div style="font-size: 13pt; text-align: center;">
    ...
    <p><b>JENIS PELAYANAN ANGKUTAN TAKSI</b></p>
    <p><b style="font-size: 12pt;">Nomor :{{$data->no_kp}}</b></p>
  </div>
  <br>
  <div>
    <p class="jstfy">Berdasarkan Surat Keputusan Gubernur Bali Nomor : <b>{{$data->no_sk_gub}}</b> Tanggal <b>{{Fungsi::bulanID($data->tgl_sk_gub)}}</b> tentang izin Operasi Penyelenggaraan Angkutan Tidak Dalam Trayek Jenis Pelayanan Angkutan Taksi Kepada : <b>{{$perusahaan->nama_perusahaan}}</b></p>
  </div>
  <br>
  ...
  <br>
  <div>
    <p class="jstfy">Oleh Kepala Dinas Perhubungan Provinsi Bali, diberikan Kartu Pengawasan Angkutan Tidak Dalam Trayek Jenis Pelayanan Angkutan Taksi dari Tanggal <b>{{Fungsi::bulanID($data->tgl_sk_gub).' s/d '.Fungsi::bulanID(date('Y-m-d', strtotime($data->tgl5." +1 year")))}}</b></p>
  ...
  <div class="row">
    <div style="float: left; width: 270px; padding: 5px; border:1px solid black;">
      ...
      <br><b>Kembali Pada : </b>
      <br><br>
      <br><b>Tanggal : {{Fungsi::bulanID($data->tgl2)}}</b>
    </div>
    ...
    <div style="float: right; width: {{$data->tgl_ditetapkan >= '2019-02-06' ? '400' : '380'}}px;">
      <br><b>{{$data->tgl_ditetapkan >= '2019-02-06' ? 'Plt.' : ''}} Kepala Dinas Perhubungan Provinsi Bali</b>
      <br><br><br><br><br>
